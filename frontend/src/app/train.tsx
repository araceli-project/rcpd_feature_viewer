...
export default function Train() {
    ...
    const [selectedTargetLabelsFile, setSelectedTargetLabelsFile] = useState<File | null>(null);
    ...
    useEffect(() => {
        if (selectedTrainingFiles && selectedTargetLabelsFile) {
            setLoading(true);
            postTrain(selectedTrainingFiles, selectedTargetLabelsFile).then((response) => {
                localStorage.setItem("Model Name", response.model_name);
                localStorage.setItem("Test Metric", response.test_metric.toString());
                localStorage.setItem("SHAP Aggregation", JSON.stringify(response.shap_agg));
    ...
    return (
        <div>
            ...
            <p className="text-gray-600 mb-4">
                This section will allow you to upload training data and train a model for a target task.
      </p>
        ...
        {!loading && trainResponse && (
            <div className="mt-4 p-4 border border-green-500 rounded bg-green-50">
                ...
                <p><strong>Model Name:</strong> {trainResponse.model_name}</p>
                <p><strong>Test Metric:</strong> {trainResponse.test_metric}</p>
                <p><strong>SHAP Aggregation:</strong> {JSON.stringify(trainResponse.shap_agg)}</p>
            </div>
        )}
    </div>
  );
